...
--isolated --from "memtomem[all]==0.6.1" memtomem-server`.
...
The npm plugin for this core version is `opencode-memtomem@0.3.2` (bundling
core `0.6.1`). Add it through OpenCode's plugin form —
`{"plugin": ["opencode-memtomem@0.3.2"]}` in `opencode.json`. The plugin
...
> Running `uvx --isolated --from "memtomem[all]==0.6.1" memtomem-server`

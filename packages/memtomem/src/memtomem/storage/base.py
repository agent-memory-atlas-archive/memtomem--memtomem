...
class StorageBackend(Protocol):
    ...
    # Metadata-only sibling: rewrites the retrieval columns (tags, validity
    # window) for hash-matched chunks without touching content, vectors or
    # timestamps (#2124, #2140).
    ...
    # Stored vectors keyed by ``str(chunk.id)``; ids without a usable vector are
    # omitted, and a store with no vector table returns ``{}``.
    ...
    async def finalize_session_summary(
        self, session_id: str, summary: str, patch: dict
    ) -> bool: ...
